...
let TREE_GLOBAL_SCALE = 0.06; // global scale multiplier (0.2 keeps ~20% of previous trees)
...
const TREE_COLLIDABLE_PERCENT = 4; // % of large trees that are solid
...
function draw(){
  ...
  for(let ty=startTy; ty<=endTy; ty++){
    for(let tx=startTx; tx<=endTx; tx++){
      ...
      if(type === 'grass'){
        // probabilistic decorative grass dot to reduce clutter
        if((hash2(tx,ty+11) % 100) < Math.round(DECOR_GLOBAL_SCALE * 100)){
